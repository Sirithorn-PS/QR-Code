...
export default function InventoryPage() {
  ...
  const openBomModal = async (product: Product) => {
    ...
    setBomLoading(true)
    try {
      let boms = await fetchProductBom(product.itemCode)
      if ((!boms || boms.length === 0) && product.parentItemCodes && product.parentItemCodes.length > 0) {
        boms = await fetchProductBom(product.parentItemCodes[0]).catch(() => [])
      }
      setBomList(boms)
    } catch (err) {
      console.error(err)
  ...
  return (
    <main className="min-h-screen bg-gray-50/50 px-4 py-8 md:px-8">
          ...
          {[
            ...
            return (
              <button
                ...
                className={`p-4 rounded-2xl border font-display flex flex-col items-center justify-center text-center transition-all duration-200 cursor-pointer ${isSelected
                                    ...
                                    {user?.role === 'admin' ? (
                                      <>
                                        <input
                                          ...
                                          className={`w-24 text-right rounded-lg border px-2.5 py-1.5 text-xs font-bold transition-all shadow-2xs focus:outline-none focus:ring-1 ${isChanged
                    ...
                    <div key={fg.id} className="w-full rounded-3xl border border-gray-200/90 bg-white p-5 sm:p-6 shadow-[0_2px_15px_rgb(0,0,0,0.02)] hover:shadow-[0_8px_30px_rgb(0,0,0,0.06)] hover:border-gray-300 transition-all flex flex-col justify-between gap-4 relative group/card font-display">
                      ...
                      <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-6">
                        ...
                        <div className="flex flex-col sm:flex-row md:flex-col lg:flex-row items-stretch sm:items-center md:items-stretch lg:items-center gap-3 sm:gap-4 shrink-0 w-full md:w-auto mt-2 md:mt-0 pt-4 md:pt-0 border-t md:border-t-0 border-gray-100">
                          <div className="bg-red-50/70 min-w-[140px] px-4 py-3 rounded-2xl border border-red-100/80 flex flex-col items-center justify-center text-center shadow-2xs">
                            <span className="text-[10px] sm:text-[11px] text-[#BE1111] font-bold uppercase tracking-wider text-center">
                              STOCK คงเหลือ
                            </span>
                            <div className="text-2xl sm:text-3xl font-black text-[#BE1111] leading-tight text-center mt-0.5">
                              {fg.quantity.toLocaleString()}
                            </div>
                      ...
                      <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-6">
                        ...
                        <div className="flex flex-col sm:flex-row md:flex-col lg:flex-row items-stretch sm:items-center md:items-stretch lg:items-center gap-3 sm:gap-4 shrink-0 w-full md:w-auto mt-2 md:mt-0 pt-4 md:pt-0 border-t md:border-t-0 border-gray-100">
                          <div className="bg-slate-50 min-w-[140px] px-4 py-3 rounded-2xl border border-slate-200 flex flex-col items-center justify-center text-center shadow-2xs">
                            ...
                            <div className="text-2xl sm:text-3xl font-black text-gray-900 leading-tight text-center mt-0.5">
      ...
      <AnimatePresence>
        {selectedBomProduct && (() => {
          const displayParentCode = bomList[0]?.parentItemCode?.trim() || selectedBomProduct.parentItemCodes?.[0] || selectedBomProduct.itemCode
          const parentProduct = products.find(p => p.itemCode.trim().toLowerCase() === displayParentCode.toLowerCase())
          const mainTitle = parentProduct?.name || (selectedBomProduct.itemCode === displayParentCode ? selectedBomProduct.name : `สูตรการผลิตสำหรับสินค้าหลัก (${displayParentCode})`)
