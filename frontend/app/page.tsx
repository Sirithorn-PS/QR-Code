...
import { ScanLine, ArrowRight, ShieldCheck, CheckCircle2, Package, Users, Shield, LayoutDashboard, UserCheck } from 'lucide-react'
...
export default function HomePage() {
  ...
  return (
    <div className="min-h-[calc(100vh-4rem)] bg-gradient-to-br from-slate-50 via-white to-red-50/20 py-4 sm:py-8 md:py-12 px-3 sm:px-6 lg:px-8">
      <div className="max-w-4xl mx-auto">
        ...
        >
          ...
          <motion.div variants={itemVariants} className="flex flex-col items-center justify-center text-center">
            {/* Red Accent Bar */}
            <div className="w-12 h-1 bg-[#BE1111] rounded-full mb-3" />
            
            {/* Brand Title: WPK MMS */}
            <div className="flex items-center gap-2">
              <span className="text-xl sm:text-2xl md:text-3xl font-bold text-[#BE1111]">
                WPK
              </span>
            ...
            {/* Brand Subtitle */}
            <p className="text-[11px] sm:text-xs text-slate-600 font-medium tracking-wide uppercase mt-1">
          ...
          <motion.div variants={itemVariants} className="mb-6 sm:mb-10 text-center">
            <h2 className="text-xl sm:text-3xl md:text-4xl font-semibold text-[#0F172A]">
              หน้าหลัก
            </h2>
            <p className="text-slate-500 mt-1 sm:mt-2 text-xs sm:text-base font-normal">
              ยินดีต้อนรับกลับมา, <span className="text-[#0F172A] font-semibold">{user?.fullName || user?.username || 'ผู้ใช้งาน'}</span>
            </p>
          </motion.div>
          ...
          <motion.div variants={itemVariants} className="grid grid-cols-2 gap-3 sm:gap-6 md:gap-8 mb-8 md:mb-12">
            {user?.role === 'admin' ? (
              <>
                {/* Admin Card 1: User Management */}
                <Link href="/users" className="group block focus:outline-none">
                  <div className="h-full flex flex-col items-center text-center bg-white/80 backdrop-blur-md border border-gray-200/60 p-3.5 sm:p-6 md:p-8 rounded-2xl sm:rounded-3xl shadow-xs hover:shadow-xl hover:bg-white transition-all duration-500 relative overflow-hidden">
                    ...
                    <div className="w-12 h-12 sm:w-16 sm:h-16 md:w-20 md:h-20 bg-red-50 text-[#BE1111] rounded-full flex items-center justify-center mb-3 sm:mb-5 md:mb-6 group-hover:scale-110 group-active:scale-95 transition-transform duration-500 relative z-10 shadow-xs border border-red-100/60 shrink-0">
                      <Users className="w-6 h-6 sm:w-8 sm:h-8 md:w-9 md:h-9 stroke-[2.2]" />
                    </div>
                    <h3 className="text-base sm:text-xl md:text-2xl font-semibold text-slate-900 mb-1 sm:mb-2 md:mb-2.5 relative z-10 whitespace-nowrap">จัดการผู้ใช้งาน</h3>
                    <p className="text-slate-500 mb-4 sm:mb-6 md:mb-8 text-[11px] sm:text-xs md:text-sm leading-tight sm:leading-relaxed relative z-10 font-normal">
                      <span className="block whitespace-nowrap">สร้างและแก้ไขข้อมูล</span>
                      <span className="block whitespace-nowrap">บัญชีพนักงานทั้งหมด</span>
                    </p>
                ...
                {/* Admin Card 2: System Dashboard */}
                <Link href="/dashboard" className="group block focus:outline-none">
                  <div className="h-full flex flex-col items-center text-center bg-white/80 backdrop-blur-md border border-red-100/60 p-3.5 sm:p-6 md:p-8 rounded-2xl sm:rounded-3xl shadow-xs hover:shadow-xl hover:bg-white transition-all duration-500 relative overflow-hidden">
                    ...
                    <div className="w-12 h-12 sm:w-16 sm:h-16 md:w-20 md:h-20 bg-red-50 text-[#BE1111] rounded-full flex items-center justify-center mb-3 sm:mb-5 md:mb-6 group-hover:scale-110 group-active:scale-95 transition-transform duration-500 relative z-10 shadow-xs border border-red-100/60 shrink-0">
                      <LayoutDashboard className="w-6 h-6 sm:w-8 sm:h-8 md:w-9 md:h-9 stroke-[2.2]" />
                    </div>
                    <h3 className="text-base sm:text-xl md:text-2xl font-semibold text-slate-900 mb-1 sm:mb-2 md:mb-2.5 relative z-10 whitespace-nowrap">แดชบอร์ดระบบ</h3>
                    <p className="text-slate-500 mb-4 sm:mb-6 md:mb-8 text-[11px] sm:text-xs md:text-sm leading-tight sm:leading-relaxed relative z-10 font-normal">
                      <span className="block whitespace-nowrap">ภาพรวมระบบและสถิติ</span>
                      <span className="block whitespace-nowrap">บัญชีผู้ใช้งานทั้งหมด</span>
                    </p>
              ...
              <>
                ...
                <Link href="/scan" className="group block focus:outline-none">
                  <div className="h-full flex flex-col items-center text-center bg-white/80 backdrop-blur-md border border-red-100/60 p-3.5 sm:p-6 md:p-8 rounded-2xl sm:rounded-3xl shadow-xs hover:shadow-xl hover:bg-white transition-all duration-500 relative overflow-hidden">
                    ...
                    <h3 className="text-base sm:text-xl md:text-2xl font-semibold text-slate-900 mb-1 sm:mb-2 md:mb-2.5 relative z-10 whitespace-nowrap">สแกนสินค้า</h3>
                    <p className="text-slate-500 mb-4 sm:mb-6 md:mb-8 text-[11px] sm:text-xs md:text-sm leading-tight sm:leading-relaxed relative z-10 font-normal">
                ...
                <Link href="/inventory" className="group block focus:outline-none">
                  <div className="h-full flex flex-col items-center text-center bg-white/80 backdrop-blur-md border border-gray-200/60 p-3.5 sm:p-6 md:p-8 rounded-2xl sm:rounded-3xl shadow-xs hover:shadow-xl hover:bg-white transition-all duration-500 relative overflow-hidden">
                    ...
                    <h3 className="text-base sm:text-xl md:text-2xl font-semibold text-slate-900 mb-1 sm:mb-2 md:mb-2.5 relative z-10 whitespace-nowrap">จัดการสต็อก</h3>
                    <p className="text-slate-500 mb-4 sm:mb-6 md:mb-8 text-[11px] sm:text-xs md:text-sm leading-tight sm:leading-relaxed relative z-10 font-normal">
          ...
          <motion.div variants={itemVariants} className="max-w-3xl mx-auto">
            <div className="mb-4 text-center">
              <h3 className="text-base sm:text-lg font-semibold text-gray-900">ภาพรวมระบบ</h3>
              <p className="text-[11px] sm:text-xs text-gray-500 mt-0.5 sm:mt-1 font-normal">สถานะการทำงานและสิทธิ์ผู้ใช้งานปัจจุบัน</p>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3.5 sm:gap-5">
              <div className="flex items-center p-3.5 sm:p-5 bg-white/70 backdrop-blur-md rounded-2xl border border-gray-200/60 shadow-xs transition-all hover:bg-white hover:shadow-md">
                <div className="w-10 h-10 sm:w-12 sm:h-12 rounded-xl bg-gray-50 border border-gray-100 text-gray-700 flex items-center justify-center shrink-0 mr-3.5 sm:mr-4 shadow-xs">
                  {user?.role === 'admin' ? (
                    ...
                    <Package className="w-5 h-5 sm:w-6 sm:h-6 text-[#BE1111]" />
                  )}
                </div>
                <div>
                  <p className="text-[10px] sm:text-[11px] text-gray-500 font-normal mb-0.5 uppercase tracking-wider">บทบาทของคุณ</p>
                  <p className="font-semibold text-gray-900 text-xs sm:text-base">
                    {user?.role === 'admin' 
                      ? 'แอดมินระบบ (System Admin)' 
                      : user?.role === 'supervisor' 
                      ? 'ผู้ควบคุมดูแลระบบ (Supervisor)' 
                      : 'พนักงานทั่วไป (Staff)'}
                  </p>
                </div>
              </div>
              ...
              <div className="flex items-center p-3.5 sm:p-5 bg-white/70 backdrop-blur-md rounded-2xl border border-gray-200/60 shadow-xs transition-all hover:bg-white hover:shadow-md">
                ...
                <div>
                  <p className="text-[10px] sm:text-[11px] text-gray-500 font-normal mb-0.5 uppercase tracking-wider">อัปเดตล่าสุด</p>
                  <div className="flex items-center gap-2">
                    <span className="relative flex h-2.5 w-2.5">
                      <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-emerald-400 opacity-75"></span>
                      <span className="relative inline-flex rounded-full h-2.5 w-2.5 bg-emerald-500"></span>
                    </span>
                    <p className="font-semibold text-gray-900 text-xs sm:text-base">ระบบพร้อมใช้งาน</p>
                  </div>
                </div>
              </div>
            </div>
