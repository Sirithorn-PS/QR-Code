          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="flex items-center p-6 bg-white/60 backdrop-blur-md rounded-3xl border border-gray-200/50 shadow-sm transition-all hover:bg-white hover:shadow-md">
              ...
              <div>
                ...
                <p className="font-bold text-gray-900 text-lg capitalize tracking-tight">{user?.role === 'admin' ? 'ผู้ควบคุมดูแลระบบ (Supervisor)' : user?.role === 'warehouse_staff' ? 'พนักงานทั่วไป' : user?.role || 'Guest'}</p>
              </div>
            </div>
