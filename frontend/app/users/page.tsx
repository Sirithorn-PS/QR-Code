    ...
    <div className="min-h-screen bg-slate-50/60 p-4 sm:p-6 lg:p-8 font-body">
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <div className="bg-white p-4 sm:p-5 rounded-3xl border border-slate-200/80 shadow-xs flex flex-col lg:flex-row gap-3 sm:gap-4 justify-between items-stretch lg:items-center">
          ...
          {/* Dropdown Filters Group: Side-by-side on mobile & desktop */}
          <div className="grid grid-cols-2 gap-2 sm:flex sm:flex-row sm:items-center sm:gap-3 w-full lg:w-auto">
            ...
            <div className="relative w-full sm:w-auto" ref={roleDropdownRef}>
              <button
                ...
                className={`w-full sm:w-auto inline-flex items-center justify-between gap-1.5 sm:gap-3 px-3 sm:px-4 py-2.5 rounded-2xl border text-xs sm:text-sm font-normal transition-all cursor-pointer ${
                  roleFilter !== 'all'
              ...
              >
                <div className="flex items-center gap-1.5 sm:gap-2 min-w-0">
                  <span className="truncate">{currentRoleOption.label}</span>
                  <span className="px-1.5 sm:px-2 py-0.5 text-[10px] sm:text-[11px] rounded-full bg-white border border-slate-200 text-slate-500 font-normal shrink-0">
                    {currentRoleOption.count}
                  </span>
                </div>
                <ChevronDown className={`w-3.5 h-3.5 sm:w-4 sm:h-4 text-slate-400 shrink-0 transition-transform duration-200 ${isRoleDropdownOpen ? 'rotate-180' : ''}`} />
              </button>
              ...
              <AnimatePresence>
                {isRoleDropdownOpen && (
                  <motion.div
                    ...
                    animate={{ opacity: 1, y: 0 }}
                    exit={{ opacity: 0, y: 5 }}
                    className="absolute left-0 top-full mt-2 w-60 sm:w-64 bg-white rounded-2xl shadow-xl border border-slate-100 p-2 z-30 space-y-1"
                  >
            ...
            <div className="relative w-full sm:w-auto" ref={statusDropdownRef}>
              ...
              >
                ...
                <ChevronDown className={`w-3.5 h-3.5 sm:w-4 sm:h-4 text-slate-400 shrink-0 transition-transform duration-200 ${isStatusDropdownOpen ? 'rotate-180' : ''}`} />
              </button>
              ...
              <AnimatePresence>
                {isStatusDropdownOpen && (
                  <motion.div
                    ...
                    animate={{ opacity: 1, y: 0 }}
                    exit={{ opacity: 0, y: 5 }}
                    className="absolute right-0 sm:left-0 sm:right-auto top-full mt-2 w-52 sm:w-56 bg-white rounded-2xl shadow-xl border border-slate-100 p-2 z-30 space-y-1"
                  >
                    <div className="px-3 py-1.5 text-[11px] font-normal text-slate-400">
                      แยกตามสถานะบัญชี
                    </div>
